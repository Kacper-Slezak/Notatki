Zapraszam do pokoju 205/IIp/B9  
PZDR  
AL  
  
W dniu 20.10.2025 o 20:02, Kacper Ślęzak pisze:

> W dniu 2025-10-20 19:25, Artur Lasoń napisał(a):  
>   
> Dziękuję za wiadomość  
> Tak przyjdę do B9 w czwartek 23/10 na godzinę 11.30.  
> Z poważaniem,  
> Kacper Ślęzak
> 
> > Panie Kacprze,  
> > Chyba trzeba pogadać. Może Pan podejść do mnie w czwartek 23/10 do B9?  
> > Około 11:30?  
> > AL  
> >   
> > W dniu 20.10.2025 o 14:55, Kacper Ślęzak pisze:
> > 
> > > Szanowny Panie Doktorze,  
> > >   
> > > Nazywam się Kacper Ślęzak i jestem studentem 3. roku Teleinformatyki na Wydziale Informatyki, Elektroniki i Telekomunikacji.  
> > >   
> > > Zwracam się do Pana z uprzejmą prośbą o rozważenie możliwości objęcia opieki promotorskiej nad moją pracą inżynierską.  
> > >   
> > > Pana specjalizacja w obszarze architektur cloud-native oraz technologii Kubernetes w pełni pokrywa się z moimi zainteresowaniami zawodowymi. To właśnie w tej tematyce chciałbym realizować swój projekt inżynierski.  
> > >   
> > > Z góry dziękuję za poświęcony czas i rozważenie mojej prośby.  
> > >   
> > > Z poważaniem,  
> > > Kacper Ślęzak  
> > > Nr albumu: 422843
> > > 


Super! Witam w ekipie :)  

Panie Kacprze, pracujemy nad tematyką, zakresem. Jeszcze nie definiujemy szcegółowo celu pracy, tytułu ale zakres/tematykę pracy. Czego chciałby się Pan nauczyć poprzez realizację pracy, jakie umiejętności są poszukiwane na rynku (tak, proszę przeglądnąć oferty pracy ...).  
  
Proszę postarać się taką tematykę ogarnąć i przesłać mi mailem to co Pan znalazł, co Pana zainteresowało.  
  
Czasu jest sporo, ale ... nie ma co odkładać.  
PZDR  
AL  
  
W dniu 27.10.2025 o 15:38, Kacper Ślęzak pisze:

> Szanowny Panie Doktorze,  
>   
> po namyśle chciałbym pisać pracę inżynierską pod Pana opieką.  
>   
> Pozdrawiam,  
> Kacper Ślęzak
> 

Super!  
AL  
  
W dniu 03.11.2025 o 14:59, Kacper Ślęzak pisze:

> Panie Doktorze,  
> Dziękuję bardzo za wiadomość i wskazówki dotyczące tematu pracy.  
>   
> Pomysł połączenia service mesh z bezpieczeństwem, a w szczególności analiza wpływu różnych algorytmów kryptograficznych na wydajność, brzmi interesująco. Chętnie podejmę się tego zadania.  
>   
> Dam znać, jak tylko dowiem się czegoś konkretnego i będę miał pierwsze wyniki rozeznania.  
>   
> Pozdrawiam, Kacper  
>   
>   
>   
> W dniu 2025-11-03 11:14, Artur Lasoń napisał(a):
> 
> > Panie Kacprze,  
> > Jeśli idziemy w service mesh to ja bym postawił na połączenie tego z  
> > security. Może jakieś słowo kluczowe z dziedziny bezpieczeństwa  
> > włożylibyśmy do tytułu pracy (ładnie w CV wygląda).  
> >   
> > Ja np. byłbym ciekawy czy jest możliwa zmiana algorytmu  
> > kryptograficznego stosowanego w servie mesh i porównanie czy bardziej  
> > obciązająca kryptografia wpływa n wydajność systemu (usługi  
> > działającej na tym klastrze z service mesh.)  
> >   
> > TEORETYCZNIE jest możliwość podmiany kryptogrfii - praktycznie nie wiem  
> > czy to nie zacznie się wykrzaczać.  
> >   
> > Co zrobić: poszukać czy ktoś podmieniał kryptografię i na jakim  
> > service meshu. 2. Zacząć samemu stawiać jakiś klaster (k3s?) na to  
> > jakiś service mesh (istio?), podmienić cypher na inny niż domyśłny -  
> > zobaczyć czy się nie wywala. 3. Poszukac co innego można zmienić w  
> > service mesh z punktu widzenia security (???)  
> >   
> > Wstęny tytuł:  
> > Analiza mechanizmów bezpieczeństwa w klastrze obliczeniowym Kubernetes (?????)  
> >   
> > Pozdrawiam,  
> > AL  
> >   
> > W dniu 03.11.2025 o 10:00, Kacper Ślęzak pisze:
> > 
> > > Panie Dokrorze ,  
> > >   
> > > Tak, jak najbardziej zgadzam się na rozwijanie Tematu 3.  
> > > Jakie sugerowałby Pan w takim razie kolejne kroki? Czy powinienem już zacząć od głębszego zapoznania się z rozwiązaniami service mesh, czy ma Pan może inne sugestie na ten początkowy etap?  
> > >   
> > > Pozdrawiam,  
> > > Kacper Ślęzak  
> > >   
> > >   
> > > W dniu 2025-11-02 20:40, Artur Lasoń napisał(a):
> > > 
> > > > Panie Kacprze,  
> > > > Temat 1. - tutaj trudno mi wymyślić kilka scenariuszy budowy takiej  
> > > > platformy, może muszę coś poczytać  
> > > > Temat 2. Klasyka :) W temacie observability zawsze jest coś do  
> > > > zrobienia. W połączeniu ze skalowaniem - duży potencjał.  
> > > > Temat 3. Klasyka + Tutaj możemy porównać różne service mesh, możemy  
> > > > zobaczyć czy da się skonfigurować inne "cyphers" w szyfrowanych  
> > > > kanałach. Service mesh - bardzo produkcyjna sprawa, moim zdaniem  
> > > > gorący temat.  
> > > >   
> > > > Rozwijalibyśmy Temat 3 ?????  
> > > >   
> > > > PZDR  
> > > > AL  
> > > >   
> > > >   
> > > > W dniu 02.11.2025 o 14:26, Kacper Ślęzak pisze:
> > > > 
> > > > > Panie Doktorze,  
> > > > >   
> > > > > dziękuję za wiadomość. Zgodnie z Pana sugestią, przeanalizowałem rynek pracy pod kątem poszukiwanych umiejętności i przygotowałem listę tematów, które mnie interesują.  
> > > > >   
> > > > > Według mnie największy potencjał rynkowy mają obecnie trzy konkretne obszary:  
> > > > >   
> > > > >      1. Inżynieria Platform: Koncepcja i budowa wewnętrznej platformy deweloperskiej w oparciu o Kubernetes i metodykę GitOps  
> > > > >   
> > > > >      2. Zaawansowana Obserwowalność: Projekt i implementacja stosu monitorującego (Prometheus, Grafana, Loki) do analizy wydajności i automatyzacji skalowania aplikacji w Kubernetesie.  
> > > > >   
> > > > >      3. Zarządzanie i bezpieczeństwo aplikacji w Kubernetesie (Service Mesh): Praktyczne wykorzystanie narzędzia Istio do inteligentnego kierowania ruchem między częściami aplikacji. Chodzi głównie o wdrożenie bezpiecznych aktualizacji (np. udostępnienie nowej funkcji najpierw tylko 10% użytkowników, tzw. Canary release) oraz o automatyczne szyfrowanie i zabezpieczanie komunikacji między nimi.  
> > > > >   
> > > > > Jest to oczywiście wynik mojej wstępnej analizy i szczerze mówiąc, nie mam jeszcze pełnego rozeznania co do skali trudności każdego z tych zadań w ramach pracy inżynierskiej.  
> > > > >   
> > > > > Będę bardzo wdzięczny za Pana radę i pomoc w wyborze odpowiedniego kierunku.  
> > > > >   
> > > > > Pozdrawiam,  
> > > > >   
> > > > > Kacper Ślęzak


Panie Kacprze,  
Super!  
Ad.1 TAK, k3s to "lekka" wersja k8s - często jej używamy.  
  
Ad.2 No to mamy pierwszy pomysł. Możemy w obecnej wersji ISTIO ustawić mTLS w wersji 1.3 i 1.2. Co ciekawe możemy wbierać pomiędzy jakimi Envoyami jest 1.3 a gdzie jest 1.2. Pozwalamy mTLS wybrać 1.3 i patrzymy co wybierze z pięciu cipher suites. Robimy pomiary wykorzystania zasobów (CPU, RAM, opóźnienie w obsłudze requesta) - mamy scenariusz referencyjny. Potem idziemy w RFC i czytamy na jakiej podstawie on dokonuje tego wyboru. Czy możemy coś pozmieniać w klastrze by wybrał inaczej? Jak możemy - robimy badania - sceanariusz drugi. Pomiędzy wbranymymi Envoy wymuszamy mTLS 1.2 i podstawiamy taki cipher suite i robimy badania. Ten scenariusz pozwala na obniżenie poziomu bezpieczeńśtwa (słaby cipher) ale pozwala na mniejsze obciążenie poda, który nie wymaga mocnego cipher suites (jakieś edge copmputing, IoT, cokolwiek). Mamy trzy scenariusze badawcze i wstępny pomysł na pracę. Coś jeszce może uda się pokonfigurwać w ISTIO (patrz Ad.3) i mamy "Adaptacyjna konfiguracja warstwy service mesh na potrzeby środowisk obliczeniowych o różnej wydajności".  
  
Ad.3 Polityki autoryzacyjne - tutaj rozumiem JWT z ustawieniem pola claim a w zakresie identyfikowania aplikacji/worloadów SPIFFE. To może nie mieć znaczenia (może być niemierzalne). Czy ustawimy coś w claim czy nie ustawimy - nie zmierzymy różnicy. Zmierzymy chyba różnicę przy ustwianiu allow-all/STRICT i modyfikacji (np. Envoye gadają tylko parami, nigdy frontend nie dobije się do bazy danych, wyłącznie front do backendu a ten do bazy). I takie wymuszanie polityki może już być mierzalne.  
  
Gdybym coś namieszał albo było niejasne proszę dać znać. Mam poczucie, że jeszce jeden krok i mamy temat/tytuł pracy.  
  
PZDR  
AL  
  
  
W dniu 21.11.2025 o 11:13, Kacper Ślęzak pisze:

> Panie Doktorze,  
>   
> Ruszyłem już z budową środowiska laboratoryjnego i mam garść pierwszych wniosków.  
>   
> 1. Kwestia narzędzia  
>   
> Postawiłem na narzędzie k3d. To w praktyce certyfikowany k3s, tylko zapakowany w kontenery Docker'a. Funkcjonalnie to to samo, a na laptopie pozwala mi to sprawnie stawiać i resetować klaster, więc uznałem, że na start będzie idealne. Nie wiem jak będzie to miało wpływ w prawdziwym badaniu, więc prosił bym o opinie  
>   
> 2. Wnioski z analizy kryptograficznej  
>   
> Zrobiłem głębszy research standardu TLS 1.3 (domyślnego w Istio). Zgodnie ze specyfikacją (RFC 8446) ten protokół jest bardzo restrykcyjny – ma tylko 5 zestawów szyfrów i wyciął stare algorytmy. Próbowałem wymusić ręczną podmianę szyfru na inny, ale zgodnie z przewidywaniami – nie powiodło się to (system to ignoruje).  
> Zamiast walczyć ze standardem cofnąć się na klastrze do TLS 1.2  gdzie chyba można podmieniać algorytmy kryptograficzne, jednak to starszy standard.  
>   
> 3. Inne zagadnienia  
>   
> Inną możliwością bezpieczeństwa są polityki autoryzacyjne. Które też można by było przebadać, na przykład ich narzut.  
> Możliwe, że są też inne możliwości o których nie pomyślałem. Dopiero się uczę wielu z tych rzeczy, więc trochę zajmuej mi to czasu oraz prawdopodbnie część możliwości nie zauważam, chciałbym prosić o opinie na temat tego co zrobiłem do tej pory.  
>   
> Z poważaniem,  
>   
> Kacper Ślęzak
> 
> 


Panie Kacprze,  
  
Dokładnie tak samo rozumiemy plan inżynierki (pkt. 1-4).  
  
Środowisko k3s (k3d) jest bardzo dobrym wyborem. Nie przenosimy się na czyste VMki.  
  
Kolejne kroki - osobiście zacząłbym od prób uruchomienia środowiska. Te próby chyba wymuszą doczytywanie dokumentacji, czyli ... teorię. Tyle, tylko że nie muszę tutaj mieć racji ;)  
  
Proszę działać i okresowo raportować jak idzie, powoli zacznę formalizować zgłoszenie Pana pracy.  
PZDR  
AL  
  
  
W dniu 24.11.2025 o 14:02, Kacper Ślęzak pisze:

> Panie Doktorze,  
>   
> Dziękuję za odpowiedź i propozycję tytułu – brzmi świetnie i w pełni go akceptuję.  
>   
> Chciałbym potwierdzić, czy dobrze zrozumiałem nakreślony plan badawczy:  
> 1.  Scenariusz referencyjny: Pozwalamy Istio na automatyczny wybór (mTLS 1.3) i zbieramy metryki.  
> 2.  Scenariusz modyfikacji 1.3: Badamy, czy da się wpłynąć na wybór parametrów wewnątrz mTLS 1.3.  
> 3.  Scenariusz "IoT" (mTLS 1.2): Wymuszamy mTLS 1.2 i słabszy szyfr, sprawdzając, czy pozwoli to odciążyć zasoby (np. CPU).  
> 4.  Opcjonalnie: Badamy narzut polityk autoryzacyjnych (np. STRICT vs ALLOW-ALL).
## Re: Praca Inżynierska[](https://poczta.agh.edu.pl/rcm-1.5/?_task=mail&_action=show&_uid=2134&_mbox=INBOX "Otwórz w nowym oknie")



Od [Artur Lasoń](mailto:alason@agh.edu.pl "alason@agh.edu.pl") w dniu 26.02.2026 11:38

[Szczegóły](https://poczta.agh.edu.pl/rcm-1.5/?_task=mail&_caps=pdf%3D1%2Cflash%3D0%2Ctiff%3D0%2Cwebp%3D1%2Cpgpmime%3D0&_uid=2134&_mbox=INBOX&_framed=1&_action=preview#headers) [Nagłówki](https://poczta.agh.edu.pl/rcm-1.5/?_task=mail&_caps=pdf%3D1%2Cflash%3D0%2Ctiff%3D0%2Cwebp%3D1%2Cpgpmime%3D0&_uid=2134&_mbox=INBOX&_framed=1&_action=preview#all-headers)

Panie Kacprze,  
Mocno idzie Pan do przodu - super!  
  
ad.1 Nigdy nie używaliśmy fortio. Do tej pory większość prac robimy na K6 - jest fajnie zintegrowane z Prometheusem i Grfaną. Ogarnięcie tych narzędzi jest też fajne samo w sobie - są to czeste rozwiązania produkcyjne. fortio - fajnie byłoby zobaczyć czym to jest, poznać nowe narzędzie (z mojego punktu widzenia). Decyzję zostawiam Panu.  
  
ad.2 Idealne dwa scenariusze - różna granularność - może różne wyniki. Róbmy obydwa przypadki.  
  
ad.3 Zdecydowanie zacznijmy od minimal, potem możemy się zastanawiać, dokładać zadania logowania ruchu, na razie minimal (ewentualnie default).  
  
ad.4 Na razie klient-server. Wprowadzenie aplikacji jest fajne, pokazuje realny scenariusz ale może mieć wpływ na wyniki. Skąd będziemy wiedzieć czy badamy wydajność klastra czy wydajność aplikacji? Pierwsze wyniki zobaczmy w najprostszym scenariuszu.  
  
Jak Panu idzie różnicowanie konfiguracji Envoya, coś udało się z szyframi, z autoryzacją?  
  
PZDR  
AL  
  
W dniu 26.02.2026 o 10:13, Kacper Ślęzak pisze:

> Panie Doktorze,  
>   
>      Zwracam się do Pana po dłuższej przerwie. Chciałbym przeprosić za brak kontaktu w ostatnich miesiącach. Ze względu na obciążenie sesją nie miałem wielu postępów w pracy. Czas ten poświęciłem na uzupełnienie wiedzy zakupiłem i realizuje kurs Kubernetesa oraz Istio. Chciałem mieć pewność, że przystępując do badań, będę dobrze rozumiał jak wszystko dokładnie działa.  
>   
>      Jeśli chodzi o bezpośrednie postępy w pracy. Wstępnie skonfigurowałem klaster (opartego na k3d) i przetestowałem narzędzia do generowania ruchu. Zanim jednak przystąpię do pomiarów, chciałbym skonsultować z Panem cztery kwestie:  
>   
> 1. Wybór narzędzia testowego:  
>      Do obciążania klastra i zbierania wyników narazie próbowałem korzystac z narzędzia fortio. Czy z Pana perspektywy to dobry wybór, czy preferuje Pan inne rozwiązanie?  
>   
> 2. Charakterystyka ruchu:  
>      Zastanawiam się, jaki rodzaj ruchu najlepiej uwydatni narzut wprowadzany przez algorytmy kryptograficzne Envoy'a. Czy powinniśmy skupić się na dużej liczbie zapytań o minimalnym rozmiarze , czy na mniejszej liczbie zapytań, ale z dużym ładunkiem danych? A może warto uwzględnić oba te scenariusze w pracy?  
>   
> 3. Profil instalacji Istio:  
>      W tutorialach powszechny jest profil demo, który ma włączone śledzenie (tracing) zapytań i bogate logowanie. Jednak jest on odradzany w testach wydajnościowych. Zastanawiam się natomiast nad wyborem między profilem default a minimal - ten pierwszy instaluje dodatkowo bramy wejściowe/wyjściowe (Ingress/Egress), podczas gdy drugi dostarcza absolutne minimum. Który z nich uważa Pan za lepszy z punktu widzenia poprawności naszych badań?  
>   
> 4. Struktura aplikacji testowej:  
>      W moich wstępnych próbach testowałem prosty układ Client-Server. Jednak wydaje mi się oczywistę, że to zbyt porstę rozwiązanie. Rozważałem wykorzystanie standardowej aplikacji mikroserwisowej Bookinfo, gdzie jedno zapytanie przechodzi przez kilka usług. Jest to aplikacja dostarczana przez Istio.  
>   
> Będę bardzo wdzięczny za Pana opinię i wskazówki.  
> Z poważaniem,  
> Kacper Ślęzak  
> 422843

Super, zmieniam zgłoszenie z wersji early draft na final - ze zgłoszaniem poczekamy może z miesiąc ale temat mamy zamknięty. Może tylko proszę ustawić sobie na poczatek maja - "sprawdzić czy moja praca jest formalnie zgłoszona"  
  
Dokumentacja - super, nic nie poprawiać, proszę działać tak dalej :) Czy nie trzeba będzie czegoś powtórzyć - nie specjalnie da się to zagwarantować.  
  
Język - zachęcałbym do j. angielskiego - wszystko co istotne dzieje się w tym języku, chyba, że bardzo niekomfortowo będzie się Pan czuł... Z mojej strony patrzę na prace w języku angielskim: "czy język czytelny, komunikacja jednoznaczna? TAK - bardzo dobrze, drobne usterki, stylistyczne nie szkodzą!"  
  
Pozdrawiam serdecznie, spokojnych Świąt także z mojej strony!  
AL  
  
  
  
W dniu 02.04.2026 o 21:38, Kacper Ślęzak pisze:

> Panie Doktorze,  
>   
> Dziękuję za wiadomość oraz przygotowanie bezpiecznego zgłoszenia tematu pracy. Po analizie uważam, że taka forma jest bardzo korzystna, ponieważ daje dużą elastyczność w doborze scenariuszy badawczych.  
>   
> Chciałbym zasięgnąć Pana rady w dwóch kwestiach:  
>   
>      1.Język pracy: Zastanawiam się nad pisaniem pracy w języku angielskim. Przemawia za tym fakt, że cała dokumentacja techniczna oraz literatura są dostępne głównie w tym języku. Z drugiej strony, pisanie w języku ojczystym daje większą swobodę. Czy z Pana perspektywy praca w którymś języku jest preferowana?  
>   
>     2.Metodyka dokumentowania badań: Przeprowadziłem już pierwsze wstępne testy.  Dokumentuję je na bieżąco, chciałbym zapytać, co mógłbym poprawić w obecnym sposobie rejestracji wyników, aby uniknąć konieczności powtarzania testów na późniejszym etapie pisania pracy? Tutaj załączam link do repozytorium z dokumentacją: [https://github.com/](https://github.com/) Kacper-Slezak/Praca-In-ynierska.git  
>   
>   
> Korzystając z okazji, życzę Panu radosnych i spokojnych Świąt Wielkanocnych.  
>   
> Z poważaniem,  
> Kacper Ślęzak  
> Nr albumu: 422843  
>   
>   
>   
> W dniu 26.03.2026 13:49, Artur Lasoń napisał(a):
> 
> > Panie Kacprze,  
> > Zaglądnąłem do notatek o Panu i doszedłem do wniosku, że Pan w  
> > zasadzie już realizuje swoją pracę :) Przygotowałem zatem >bezpieczne<  
> > zgłoszenie Pana pracy - różne rzeczy można zrobić by wypełnić to co  
> > obiecuje proponowany tytuł i opis.  
> >   
> > Proszę spokojnie przeczytać i zastanowić się, czy takie zgłoszenie nie  
> > byłoby dla nas wygodne. Możemy zrobić to albo tamto - czyli to co się  
> > uda zrobić :)  
> >   
> > Pozdrawiam serdecznie,  
> > AL  
> >   
> >   
> > W dniu 04.03.2026 o 08:28, Kacper Ślęzak pisze:
> > 
> > > Panie Doktorze,  
> > > Zgodnie z Pana sugestiami, do testów użyję narzędzia K6, postawię Istio w profilu minimal i zostanę przy prostej architekturze Client- Server.  
> > >   
> > > Odpowiadając na Pana pytanie o różnicowanie konfiguracji Envoya – obecnie mocno walczę z wymuszeniem komunikacji po mTLS w wersji 1.2 i narzuceniem słabszych szyfrów. Idzie to dość opornie ze względu na wbudowane mechanizmy bezpieczeństwa Istio. Podjąłem się między innymi wstrzykiwania konfiguracji przez EnvoyFilter, aby zmniejszyć mtls do 1.2 . Przy którym Zderzyłem się z odrzucaniem połączeń ze względu na niezgodność wymuszanego szyfru z domyślnymi certyfikatami RSA generowanymi przez Istio.  
> > >   
> > > Narazie będę próbować skonfigurować Envoy Filter. Będę informować o dalszych postępach.  
> > >   
> > > Pozdrawiam,  
> > > Kacper Ślęzak  
> > > 422843  
> > >   
> > > W dniu 26.02.2026 11:38, Artur Lasoń napisał(a):
> > > 
> > > > Panie Kacprze,  
> > > > Mocno idzie Pan do przodu - super!  
> > > >   
> > > > ad.1 Nigdy nie używaliśmy fortio. Do tej pory większość prac robimy na  
> > > > K6 - jest fajnie zintegrowane z Prometheusem i Grfaną. Ogarnięcie tych  
> > > > narzędzi jest też fajne samo w sobie - są to czeste rozwiązania  
> > > > produkcyjne. fortio - fajnie byłoby zobaczyć czym to jest, poznać nowe  
> > > > narzędzie (z mojego punktu widzenia). Decyzję zostawiam Panu.  
> > > >   
> > > > ad.2 Idealne dwa scenariusze - różna granularność - może różne wyniki.  
> > > > Róbmy obydwa przypadki.  
> > > >   
> > > > ad.3 Zdecydowanie zacznijmy od minimal, potem możemy się zastanawiać,  
> > > > dokładać zadania logowania ruchu, na razie minimal (ewentualnie  
> > > > default).  
> > > >   
> > > > ad.4 Na razie klient-server. Wprowadzenie aplikacji jest fajne,  
> > > > pokazuje realny scenariusz ale może mieć wpływ na wyniki. Skąd  
> > > > będziemy wiedzieć czy badamy wydajność klastra czy wydajność  
> > > > aplikacji? Pierwsze wyniki zobaczmy w najprostszym scenariuszu.  
> > > >   
> > > > Jak Panu idzie różnicowanie konfiguracji Envoya, coś udało się z  
> > > > szyframi, z autoryzacją?  
> > > >   
> > > > PZDR  
> > > > AL  
> > > >   
> > > > W dniu 26.02.2026 o 10:13, Kacper Ślęzak pisze:
> > > > 
> > > > > Panie Doktorze,  
> > > > >   
> > > > >      Zwracam się do Pana po dłuższej przerwie. Chciałbym przeprosić za brak kontaktu w ostatnich miesiącach. Ze względu na obciążenie sesją nie miałem wielu postępów w pracy. Czas ten poświęciłem na uzupełnienie wiedzy zakupiłem i realizuje kurs Kubernetesa oraz Istio. Chciałem mieć pewność, że przystępując do badań, będę dobrze rozumiał jak wszystko dokładnie działa.  
> > > > >   
> > > > >      Jeśli chodzi o bezpośrednie postępy w pracy. Wstępnie skonfigurowałem klaster (opartego na k3d) i przetestowałem narzędzia do generowania ruchu. Zanim jednak przystąpię do pomiarów, chciałbym skonsultować z Panem cztery kwestie:  
> > > > >   
> > > > > 1. Wybór narzędzia testowego:  
> > > > >      Do obciążania klastra i zbierania wyników narazie próbowałem korzystac z narzędzia fortio. Czy z Pana perspektywy to dobry wybór, czy preferuje Pan inne rozwiązanie?  
> > > > >   
> > > > > 2. Charakterystyka ruchu:  
> > > > >      Zastanawiam się, jaki rodzaj ruchu najlepiej uwydatni narzut wprowadzany przez algorytmy kryptograficzne Envoy'a. Czy powinniśmy skupić się na dużej liczbie zapytań o minimalnym rozmiarze , czy na mniejszej liczbie zapytań, ale z dużym ładunkiem danych? A może warto uwzględnić oba te scenariusze w pracy?  
> > > > >   
> > > > > 3. Profil instalacji Istio:  
> > > > >      W tutorialach powszechny jest profil demo, który ma włączone śledzenie (tracing) zapytań i bogate logowanie. Jednak jest on odradzany w testach wydajnościowych. Zastanawiam się natomiast nad wyborem między profilem default a minimal - ten pierwszy instaluje dodatkowo bramy wejściowe/wyjściowe (Ingress/Egress), podczas gdy drugi dostarcza absolutne minimum. Który z nich uważa Pan za lepszy z punktu widzenia poprawności naszych badań?  
> > > > >   
> > > > > 4. Struktura aplikacji testowej:  
> > > > >      W moich wstępnych próbach testowałem prosty układ Client- Server. Jednak wydaje mi się oczywistę, że to zbyt porstę rozwiązanie. Rozważałem wykorzystanie standardowej aplikacji mikroserwisowej Bookinfo, gdzie jedno zapytanie przechodzi przez kilka usług. Jest to aplikacja dostarczana przez Istio.  
> > > > >   
> > > > > Będę bardzo wdzięczny za Pana opinię i wskazówki.  
> > > > > Z poważaniem,  
> > > > > Kacper Ślęzak  
> > > > > 422843